...
function crearFila(index) {
 ...
 // Agregar botón de eliminarproducto
...
function actualizarFila(imagen, nombre, precio, index) {
    nuevafila.innerHTML = `
...
eliminarBtn.innerText = 'Eliminar';
eliminarBtn.addEventListener('click', () => eliminarProducto(index));
nuevafila.innerHTML += `<td></td>`; // Agrega una celda vacía
nuevafila.appendChild(eliminarBtn);
...
function eliminarProducto(index) {
    const filaAEliminar = carritocompra.childNodes[index];
    if (filaAEliminar) {
        carritocompra.removeChild(filaAEliminar);
    } else {
    ...
    carritocompra.removeChild(carritocompra.childNodes[index]);

    // Eliminar el producto del array carritolocal
...
card.addEventListener("click", () => {
    ...
    actualizarFila(imagencard1, nombreproductocard1, preciocard1, carritolocal.length -1);
