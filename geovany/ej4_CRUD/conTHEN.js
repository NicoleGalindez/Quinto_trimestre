...
function agregarEstudianteDesdeModal() {
  ...
  if (nombre && !isNaN(nota1) && !isNaN(nota2) && !isNaN(nota3)) {
    if (isEditing) {
      //  guardar el estudiante editado
      guardarEstudianteEditado().then(() => {
        isEditing = false; // Salir de edición
        cerrarModal();
      }).catch((error) => {
        alert(error);
      });
...
function agregarEstudiante(nombre, nota1, nota2, nota3, definitiva) {
  return new Promise((resolve, reject) => {
    ...
    celda7.innerHTML = `
      <button class='botoncitos' class="editing" onclick="editarEstudiante(this)">Editar</button>
...
function guardarEstudianteEditado() {
  return new Promise((resolve, reject) => {
    ...
    if (nombre && !isNaN(nota1) && !isNaN(nota2) && !isNaN(nota3)) {
      ...
      cerrarModalEdit();
      resolve("Estudiante editado correctamente");
    } else {
